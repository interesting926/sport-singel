...
{
  //IR_Rev_Handler();
	if(GPIO_ReadInputPin(RPM1_Port,RPM1_Pin)==RESET)
	{
		rpm.int_flg=1;	//触发中断标志
	}
	else if(GPIO_ReadInputPin(RPM2_Port,RPM2_Pin)==RESET)
	{
...
{
	if(GPIO_ReadInputPin(MP3_BUSY_PORT,MP3_BUSY_PIN)!=RESET)
	{
  	//mp3.busy=0;
  }
}
...
/**
  ...
  */
	...
	if(TIM2_GetITStatus(TIM2_IT_UPDATE)!=RESET)	//中断溢出
	{
...
/**
 ...
 INTERRUPT_HANDLER(UART1_RX_IRQHandler, 18)
 {
    ...
    if(UART1_GetITStatus(UART1_IT_RXNE)!=RESET)
    {
		...
		{
			rdata = UART1_ReceiveData8();	//非接收中断时，需要通过接收来清除
		}
		#endif
 }
...
/**
 ...
 INTERRUPT_HANDLER(UART4_RX_IRQHandler, 18)
 {
 	#ifdef	USING_UART4
 		u8 rdata;
    if(UART4_GetITStatus(UART4_IT_RXNE)!=RESET)
    {
        UART4_ClearITPendingBit(UART4_IT_RXNE);
        rdata= UART4_ReceiveData8();
				Uart4_Receive_Data(rdata);
    }
		else
		{
			rdata= UART4_ReceiveData8();
...
INTERRUPT_HANDLER(TIM6_UPD_OVF_TRG_IRQHandler, 23)
 {
  if(TIM6_GetITStatus(TIM6_IT_UPDATE)!=RESET)
  {
